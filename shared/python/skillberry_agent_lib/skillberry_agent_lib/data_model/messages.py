...
class ParticipantMessageBase(BaseModel):
    ...
    def validate(self):  # NOTE: It would be better to do this in the Pydantic model
        ...
        if not (self.has_text_content() or self.is_tool_call()):
            ...
            # )
            logger.error(f"Message must have either content or tool calls. Got {self}")            
            self.content = "Message without content has been generated. Adding fake content to prevent downstream errors."
